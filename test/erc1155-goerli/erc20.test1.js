...
describe('eth test', async () => {
    ...
    it("trader buy", async () => {
        ...
        const poolAddress = "0xA412Cc323B3c8e167E005E2E8049e68Bb5835f19";
        console.log("poolAddress:",poolAddress);
        expect(await erc1155Contract1.balanceOf(poolAddress,1)).to.equal(1);
        console.log("buy balance:",await erc20Contract.balanceOf(buyAddress))
        await erc20Contract.approve(pairrouterAddress, 60000000000)

        ////////////////////// buy test robustswapethforspecificNFTs
        const maxCost = 60000000000
        const swapList = [[[poolAddress, 10], maxCost]]
        const ddl = (await ethers.provider.getBlock("latest")).timestamp * 2;
        const robustBuy = await pairrouterContract.robustSwapERC20ForAnyNFTs(swapList, maxCost, buyAddress, ddl)
